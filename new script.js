...
const fetchChatTags = async () => {
    try {
        const response = await fetch('http://127.0.0.1:9000/get_chattags_keywords/', {
            method: 'GET',
            headers: {
...
const fetchChatQuestions = async () => {
    try {
        const response = await fetch('http://127.0.0.1:9000/get_chattags_questions/', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'xapi': '1234'
...
const fetchKeywords = async () => {
    try {
        const response = await fetch('http://127.0.0.1:9000/get_keywords/', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'xapi': '1234'
...
const fetchQuestions = async () => {
    try {
        const response = await fetch('http://127.0.0.1:9000/get_questions/', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'xapi': '1234'
...
const generateResponse = async (chatElement) => {
    ...
    const apiUrl = 'http://127.0.0.1:9000/get_response/?msg=' + encodeURIComponent(userMessage);
...
const showSuggestions = () => {
    ...
    if (matchingQuestions.length > 0) {
        matchingQuestions.forEach(suggestion => {
            ...
            listItem.textContent = suggestion.suggest_questions.trim(); // Show the question
